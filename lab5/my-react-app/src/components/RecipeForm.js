...
function RecipeForm({ currentRecipe, onSave }) {
  ...
  const [category, setCategory] = useState("М'ясні страви");
  const [ingredients, setIngredients] = useState('');
  const [instruction, setInstruction] = useState('');
  const [image, setImage] = useState('');

  // Ефект для заповнення форми при редагуванні
  useEffect(() => {
    if (currentRecipe) {
      setTitle(currentRecipe.title);
      setTime(currentRecipe.time);
      setCategory(currentRecipe.category || "М'ясні страви");
  ...
  const handleSubmit = (e) => {
    ...
    // Передаємо дані на onSave (яка в App.js тепер шле fetch на сервер)
    onSave({
      // Якщо ми редагуємо, передаємо id, щоб сервер зробив UPDATE, а не INSERT
      id: currentRecipe?.id || null, 
      title,
      time: time, // Залишаємо як у 4 лабі, або Number(time) для сортування за варіантом 25
      category,
      ingredients,
      instruction,
      ...
      <form className="recipe-form" onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <textarea 
            ...
            placeholder="Перерахуйте інгредієнти..." 
            required
          ></textarea>
        </div>
